...
def marzullo_algorithm(ranges):
    ...
    for i in range(len(table) - 1):
        # Current number of overlapping intervals
        cnt = cnt - table[i][1]
        #print 'i: ',i, table[i][1], 'Cnt: ', cnt, 'Best: ', best,
        print "i : %d, table[i][1] : %2d, Cnt : %d, Best : %d," % (i, table[i][1],
            cnt, best),
        if best < cnt:
            best = cnt
            beststart = table[i][0]
            bestend   = table[i+1][0]
        print "beststart : %2d, bestend : %2d, current best : %2d"%(beststart,
                bestend, best)
    print "********************************"
